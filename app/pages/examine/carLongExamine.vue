...
<script>
	import {queryUserById,commonExamine,carLongPictureUp,carLongPictureExamine,queryLongPictureById,ofMe,carLongBaoAnExamine} from "../../api/request.js"
	export default {
		onLoad(data) {
		...
		methods: {
			queryLongPictureById(id){
				queryLongPictureById(id).then(resp=>{
			...
			isagree(opinion){
				if(this.loginUser.deptId==6){
					...
					carLongPictureExamine(this.examineInfo).then(resp=>{
					...
					carLongBaoAnExamine(this.examineInfo).then(resp=>{
						if(resp.data.code==200){
							uni.showToast({
								icon:"success",
								title:"审批成功"
