<template>
	<view>
		...
		<view class="item">
			...
			<view class="content">{{examineInfo.visiteStartTime}}</view>
		</view>
		<view class="item">
			<view class="title">访问结束时间</view>
			<view class="content">{{examineInfo.visiteEndTime}}</view>
		</view>
		<view class="item">
...
<script>
	import {queryDeptById,queryUserById,commonExamine} from "../../api/request.js"
	export default {
		onLoad(data) {
		...
		methods: {
			isagree(opinion){
				...
				commonExamine(this.examineInfo).then(resp=>{
